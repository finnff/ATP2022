...
def generate_valid_test_cases(num_cases=10):
    # Constants
    MAX_VEHICLE_SPEED = 30
    MIN_VEHICLE_SPEED = 0.1
    MAX_VOORLIGGER_SPEED = 29
    MIN_VOORLIGGER_SPEED = 0.1
    MAX_VEHICLE_ACCELERATION = 0.2
    MIN_VEHICLE_ACCELERATION = -0.3
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("test_cases.log"), logging.StreamHandler()],
)
...
if __name__ == "__main__":
    ...
    logging.info(f"Failed cases: {len(fail_list)}, / {len(fail_list) + len(pass_list)}")
    logging.info(f"Passed cases: {len(pass_list)}, / {len(fail_list) + len(pass_list)}")
