...
# Sensoren:
## VehicleSpeedSensor
    Omdat de snelheid van een voortuig afhangt van hoe vaak die wielen roteren en hoe groot die wielen zijn, bestaat deze abstracte sensor uit een WheelSpeedSensor en een VehicleSpeedCalculator
...
De frequentie van deze veranderingen kan vervolgens door bv. een microcontroller (of onze C++/Python binding, zie VehicleSpeedCalculator) samen met informatie over de grootte van het wiel en het aantal magneten op het encoderwiel (Counts Per Revolution = CPR) omgezet worden naar een snelheid in meters per seconde.
...
### C++ Code
Normaal zou dit door een apparte microcontroller, of de CPU zelf gedaan wordt, maar dit word in C++ nageboost.
C++ code die de High/Low current outputs van de TLE5041plusC afleest, de frequentie/interval tussen deze outputs opmeet, en samen met de gegeven constanten Wieldiameter en magnetic wheelencoder CPR kan de VehicleSpeedCalculator de snelheid van het voertuig berekenen.
...
- **Gaspedaal:** regelt de acceleratie van het voertuig (verhoogt VehicleSpeed)
    + Met interface: 0-100% waar 100 versnellen is met de maximale acceleratie constante

- **Remsysteem:** regelt het afremmen van het voertuig (vermindert VehicleSpeed)
...
# ```
-->
# Groepen Constants voor simulatie. 
...
- WheelSpeedSensor (Infineon TLE5041plusC): Meet de veranderingen van het magnetisch veld van de wielen, en stuurt deze door naar de VehicleSpeedCalculator.
- VehicleSpeedCalculator (C++ code met python Bindings.): Berekent de VehicleSpeed op basis van de WheelSpeedSensor, Wieldiameter, CPR van het encoderwiel. Stuurt de VehicleSpeed door naar de CPU.
...
- De WheelSpeedSensor zit via 12v current Modulation interface verbonden met de VehicleSpeedCalculator.
- De Front Radar Sensor en de VehicleSpeedCalculator zijn verbonden met de CPU via een bekabelde interface.
...
### Diagram
![joplin render](images/classscreen.png){width=100%}
...
1. VehicleSpeedSensor: Deze klasse is verantwoordelijk voor het meten van de wielsnelheid. De klasse heeft twee private variabelen, CPR en WHEEL_DIAMETER, die respectievelijk het aantal counts per revolution van de wielencoder en de wieldiameter in meters bevatten.
2. VehicleSpeedCalculator: Deze klasse ontvangt de wielsnelheid van de VehicleSpeedSensor en berekent de voertuigsnelheid op basis van de wielsnelheid. De berekening is een puur functionele operatie.
...
7. In dit diagram zijn er relaties tussen de verschillende klassen, die aangeven hoe gegevens worden doorgegeven en gebruikt:
    - VehicleSpeedSensor stuurt gegevens naar VehicleSpeedCalculator.
    - VehicleSpeedCalculator stuurt gegevens naar PureFunctions.
...
# also add in abstraction regarding Acceleration/DECEeleration  (negative Acceleration= breaking)
...
Unit tests:
Unit Test - Wheel Speed Sensor:
De wheel speed sensor is een cruciale component in het ACCS, omdat het de snelheid van het voertuig meet en doorgeeft aan het systeem. We zullen meerdere unit tests uitvoeren om de nauwkeurigheid en betrouwbaarheid van de snelheidsmetingen te garanderen. Deze testen zullen onder andere het volgende omvatten:

Testen of de sensor correct de snelheid meet bij verschillende snelheden en omstandigheden.
Testen of de sensor correct omgaat met het signaal van het magnetische veld en of deze het juiste aantal pulsen genereert.
Testen of de sensor zichzelf goed kalibreert na 5 pulsen.
...
Integratietests:
Integratietest - Gaspedaal en remsysteem interface:

We zullen de interface tussen het ACCS en de actuatoren (gaspedaal en remsysteem) testen. De integratietesten zijn bedoeld om te verifiëren dat de communicatie tussen de software en hardware correct functioneert en dat de actuatoren reageren op de commando's van het systeem. We zullen de volgende testen uitvoeren:

Testen of het gaspedaal en het remsysteem correct reageren op commando's van het ACCS, zoals het verhogen of verlagen van de snelheid.
Testen of de actuatoren binnen de verwachte tijdsperiode reageren op commando's.
Testen of de maximale acceleratie- en vertragingconstanten correct worden toegepast.
