...
Deze sensor is getest met de volgende 3 Autos:
...
De enige fouten die ik ben tegen gekomen traden op wanneer ik handmatig (via de simulator) de snelheid van het voertuig ging veranderen, terwijl de berekening van de oude snelheid al was gestart (Multi threaded concurreny ):
...
Het resultaat is dat de code hiervoor werd samengevoegd, deze is later nog omgeschreven naar C++ voor de binding. We sturen ook nog twee integers naar de C++ binding (CPR en diameter), zodat we de hele berekening hierin kunnen uitvoeren.
...
Zoals te zien is in de loguitvoer hieronder en in `/UnitTest/removedInternalError.log`, verloopt het process zelfs met C++ niet echt snel (varieerde tussen 5000-9000 Hz voor alleen de read_speed()-operatie). Dit zou waarschijnlijk ook onvoldoende zijn geweest als ik het op mijn oorspronkelijke manier, met HI- LO] signalen. Om dit wel te laten werken hadden we geen Redis moeten gebruiken, omdat deze te langzaam is zelfs met een C++ binding.
...
## Systeemtest: Adaptive Cruise Control Systeem (ACCS) houdt Veilige afstand in verschillende verkeersscenario's

### Motivatie

*Het belangrijkste doel (kwaliteitscriterium) van het ACCS is het behouden van een veilige afstand tot het voorliggende voertuig. Het testen van het systeem in verschillende verkeerssituaties en snelheden helpt om te waarborgen dat het systeem correct functioneert, voldoet aan de gestelde kwaliteitseisen, en bovenal betrouwbaar en veilig is.*
...
We hebben een Proportional–integral–derivative (PID) systeem geïmplementeerd voor de controle van onze auto. Echter, het is zo slecht afgesteld dat het alleen effectief is voor willekeurige test cases in de Ferrari. De auto's reageren te verschillend om dezelfde set PID-waarden te gebruiken. Ik ben hieraan begonnen en heb zelfs een PyTorch-implementatie met een neuraal netwerk gebruikt, dat ongeveer 15 minuten nodig had om geschikte waarden te vinden. Desondanks is het me niet gelukt om het systeem veilig te laten werken voor alle drie de auto's. Omdat het wel is gelukt met de Ferrari, lijkt dit meer een kwestie van tijd dan een systeemfout.

Zoals de zien op het dashboard, houd de auto een veilige afstand ookal verandert de snelheid van de voorligger, het GasPedaal niveau is proportioneel aan de gemeten afstand tot de voorligger.

![dashboard gif](./ezgif-3-763dc54088.gif)


We hebben ook een simulator ontwikkeld die aangepaste test cases kan uitvoeren. De invoer hiervoor is een string-gebaseerde DSL (Domain Specific Language) die elk test case kan vertegenwoordigen. Elke actie in de de test is kan weergegeven  worden als een string.
...
zo is : `([9.46, 0.01, 14.24, 493.66], ['mV=5.41', 'p=4.61', 'mV=1.54', 'p=4.08', 'cc', 'R'])` een valide test scenario dat automatisch gegenereerd kan worden. Zie ./SysteemTest/random_test_cases.log voor een voorbeeld van deze testgevallen (en de code om deze te genereren).
